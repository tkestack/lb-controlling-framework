...
docker-build:
	...
	docker run --rm -v "$(mkfile_dir)":/go/src/tkestack.io/lb-controlling-framework \
	-w /go/src/tkestack.io/lb-controlling-framework \
	-e GO111MODULE=off \
	golang:1.12 \
	make build
...
.PHONY: build
build:
	go build -o $(OUTPUT_PATH) tkestack.io/lb-controlling-framework/cmd/lbcf-controller
